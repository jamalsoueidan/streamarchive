...
const cachedSearchRecordings = unstable_cache(
  async (
  ...
  ["streamarchive-recordings-cached"],
  { revalidate: 900 },
);
...
export async function fetchCachedRecordings({
  ...
  if (scope === "all") {
    return cachedSearchRecordings("", filterKey, sort, page, pageSize);
  }
  ...
  return cachedSearchRecordings(followerFilter, filterKey, sort, page, pageSize);
}
