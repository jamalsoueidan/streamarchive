...
export async function POST(request: NextRequest) {
  try {
    ...
    if (
      ...
      const plan = MOLLIE_PRICES[billingCycle] || MOLLIE_PRICES.monthly;
      const interval = plan.interval;

      const baseUrl =
        process.env.NEXT_PUBLIC_BASE_URL || "http://localhost:3000";

      // Schedule the first recurring charge AFTER the period the first
      // payment already covered. Without an explicit startDate, Mollie
      // defaults to "today" and charges the mandate again immediately —
      // resulting in a duplicate charge minutes after sign-up.
      const startDate = calculateEndDate(interval).slice(0, 10);
